...
time '48h'
        ...
        if( params.MapMethod == 'Minimap2' && "${Technology}" == 'ONT' ) 
            """
            minimap2 -ax map-ont ARS-bov-ont.mmi -t $task.cpus $params.QC_Dir/${SampleID}_${Technology}/${SampleID}.fastq.gz --MD | samtools sort -@ $task.cpus - -o ${SampleID}.sorted.bam 

            samtools index -@ $task.cpus ${SampleID}.sorted.bam                       
            """
        else if( params.MapMethod == 'Minimap2' && "${Technology}" == 'PB') 
            """
            minimap2 -ax map-pb ARS-bov-pb.mmi -t $task.cpus $params.QC_Dir/${SampleID}_${Technology}/${SampleID}.fastq.gz --MD | samtools sort -@ $task.cpus - -o ${SampleID}.sorted.bam 
