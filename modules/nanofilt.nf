...
maxRetries 3
        ...
        tuple val( SampleID_LR ), val( Technology ), val (Kit), path ("${SampleID_LR}.fasta.gz"), path ("${SampleID_LR}_dummy.fastq.gz"), emit: LR_sample
        ...
        if ("${Shortread_Avail}" == 'FALSE' && "${Technology}" == 'ONT')
                ...
                ln -s ${SampleID_LR}.fasta.gz ${SampleID_LR}_dummy.fastq.gz
